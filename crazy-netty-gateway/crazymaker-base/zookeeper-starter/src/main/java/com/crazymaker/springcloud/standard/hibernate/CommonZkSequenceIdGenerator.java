 ...
 * build by 尼恩 @ 疯狂创客圈
 **/
public class CommonZkSequenceIdGenerator extends IncrementGenerator {

    /**
     * 生成器的 map 缓存
     * key 为 PO 类名,value 为分布式ID生成器
     */
    private static Map<String, ZkSequenceIdGenerator> generatorMap = new LinkedHashMap<>();
    ...
    public static synchronized IdGenerator getFromMap(String type) {
         ...
         */
        ZkSequenceIdGenerator idGenerator = new ZkSequenceIdGenerator(type);
